        ...
        vi dis(n,1e9);
         dis[src]=0;
          ...
          while(pq.size()>0){
            ...
            int par=pq.front().second.first;
            int stops=pq.front().second.second;
            pq.pop();
            rep(i,adj[par].size(),0){
                int child=adj[par][i].first;
                if(dis[child]>dist+adj[par][i].second && stops<=k){
                       dis[child]=dist+adj[par][i].second;
                       pq.push({dis[child],{child,stops+1}});
                }
            }
          }
