...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 void dfs(int par,vector<vector<pi>>&adj, vi &is_visited,int last,int &count,int curr){
    is_visited[par]=1;
    count=max(count,curr);
    for(int i=0;i<adj[par].size();i++){
        int child=adj[par][i].second;
        int pos=adj[par][i].first;
        if(!is_visited[child]){
          dfs(child,adj,is_visited,adj[par][i].first,count,adj[par][i].first<last?curr+1:curr);
        }
    }
 }
 void solve(){
...
dfs(1,adj,is_visited,-1,count,1);
cout<<count<<endl;
