...
int binaryToDecimal(string n){string num = n;ll dec_value = 0;int base = 1;int len = num.length();for(int i = len - 1; i >= 0; i--){if (num[i] == '1')dec_value += base;base = base * 2;}return dec_value;}
...
public:
    int matrixScore(vector<vector<int>>& grid) {
        ...
        // counting col vise if the number of zeroes are more than it will be beneficial
        rep(j,n){
            int count_zero=0;
            int count_one=0;
            rep(i,m){
                if(grid[i][j])count_one++;
                else count_zero++;
            }
        ...
        rep(i,m){
            string add= "";
            rep(j,n){
                add+=to_string(grid[i][j]);
            }
            ans+=binaryToDecimal(add);
        }
        return ans;
