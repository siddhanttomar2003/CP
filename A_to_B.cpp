...
void solve(){
    ...
    inll(k);
    ll disx=abs(x2-x1),disy=abs(y2-y1),disz=abs(z2-z1);
    vl dis;
    dis.pb(disx);dis.pb(disy);dis.pb(disz);
    sort(dis);
    if(dis[2]/k>dis[0]+dis[1]){
       ll times=(dis[2]+k-1)/k;
       ll rem=times-(dis[0]+dis[1]);
       if(rem%2==0){
          cout<<dis[2]+dis[1]+dis[0]+(rem)<<endl;
       }
       else {
        cout<<dis[2]+dis[1]+dis[0]+(rem-1)<<endl;
       }
    }
    else {
        cout<<dis[2]+dis[1]+dis[0]<<endl;
    }
