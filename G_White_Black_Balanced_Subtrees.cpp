...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 pair<int,int> dfs(vector<vector<int>>&adj,vector<int> &is_visited,int curr, int &ans,map<int,char> &mp){
    is_visited[curr]=1;
    pair<int,int>temp={0,0};
    if(mp[curr]=='B')temp.first=1;
    else  temp.second=1;
    if(adj[curr].size()==0){
    ...
    for(int i=0;i<adj[curr].size();i++){
        if(!is_visited[adj[curr][i]]){
            pair<int,int>tepo;
           tepo= dfs(adj,is_visited,adj[curr][i],ans,mp);
           prev.first+=tepo.first;prev.second+=tepo.second;
              
    }
    }
    temp.first+=prev.first;temp.second+=prev.second;
