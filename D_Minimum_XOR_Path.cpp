...
void dfs(int start, vector<vector<pair<int,ull>>> &adj, ull &ans, ull curr, int n , vl &vis){
       if(start==n){
        ans=min(ans,curr);
        return;
       }
       for(int i=0;i<adj[start].size();i++){
         ...
         ull x=adj[start][i].second;
         if(!vis[child]){
            vis[child]=1;
            dfs(child,adj,ans,curr^x,n,vis);
            vis[child]=0;
         }
       }
}
void solve(){
    inint(n);
    inint(edges);
    vector<vector<pair<int,ull>>>adj(n+1);
    rep(i,edges,0){
        inll(a);
        inll(b);
        ull w;cin>>w;
