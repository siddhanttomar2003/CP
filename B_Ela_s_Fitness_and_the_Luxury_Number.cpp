...
// vector<int>Seg_tree(4*n,0);
  ...
  ll middle=las-fir-1;
  ll ans=0;
  if(middle>=1)ans+=(middle*3);
//   cout<<fir<<" "<<las<<endl;
//   cout<<ans<<endl;
   if(fir*fir>=a && fir*fir<=b)ans++;
   if(((fir*fir)+fir)>=a && ((fir*fir)+fir)<=b)ans++;
   if(((fir*fir)+2*fir)>=a && ((fir*fir)+2*fir)<=b)ans++;
   if(fir!=las){
    if((las*las)>=a && (las*las)<=b)ans++;
   if(((las*las)+las)>=a && ((las*las)+las)<=b)ans++;
   if(((las*las)+(2*las))>=a && ((las*las)+(2*las))<=b)ans++;
   }
  cout<<ans<<endl;
