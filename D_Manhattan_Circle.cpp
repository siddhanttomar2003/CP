...
int32_t main()
{
    ...
    int start_length_row=-1;
    int start_length_col=-1;
    bool flag1=false;
    rep(i,n,0){
        rep(j,m,0){
            if(v[i][j]=='#'){
    ...
    // cout<<start_length_row<<" "<<start_length_col<<endl;
    int l=0;
    int i=start_length_row;
    while( i<n  && v[i][start_length_col]=='#'){
        l++;
        i++;
    }
    int start_width_row=-1;
    int start_width_col=-1;
    bool flag2=false;
    rep(j,m,0){
        rep(i,n,0){
            if(v[i][j]=='#'){
    ...
    int c=0;
    int j=start_width_col;
    while(j<m && v[start_width_row][j]=='#'){
        c++;
        j++;
    }
    // cout<<l<<" "<<c<<endl;
   int start_row=start_length_row+1;
   int start_col=start_width_col+1;
   cout<<start_row+l/2<<" "<<start_col+c/2<<endl;
