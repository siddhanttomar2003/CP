...
class Solution {
public:
    int cal(string s1, string s2, int i, int j, vvi &dp){
        if(i==0 || j==0)return 0;
        if(dp[i][j]!=-1)return dp[i][j];
        int t=0;
        if(s1[i-1]==s2[j-1]){
      t=1+cal(s1,s2,i-1,j-1,dp);
        }
        int nt1=0;int nt2=0;
        nt1=cal(s1,s2,i-1,j,dp);
        nt2=cal(s1,s2,i,j-1,dp);
        return dp[i][j]=max(t,max(nt1,nt2));
    ...
    int minInsertions(string s) {
        ...
        //   vvi dp(n+1,vi(n+1,-1));
         ...
         string s2=s;
    //   int len=   cal(s1,s2,n,n,dp);
    int len=0;
        vvi dp(n+1,vi(n+1,0));
        rep(i,n+1,1){
            rep(j,n+1,1){
                int t=0;int nt1=0;int nt2=0;
                if(s1[i-1]==s2[j-1]){
                    t=1+dp[i-1][j-1];
                }
                nt1=dp[i-1][j];
            nt2=dp[i][j-1];
            dp[i][j]=max(t,max(nt1,nt2));
            len=max(len,dp[i][j]);
            }
        }
       return n-len;
