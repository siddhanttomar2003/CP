...
void solve(){
    ...
    vl p,np;
    rep(i,n,0){
        if(is_prime[v[i]])p.pb(v[i]);
        else np.pb(v[i]);
    }
    sort(p);sort(np);
    // reverse(all(p));reverse(all(np));
    int s1=p.size();int s2=np.size();
    // cout<<p<<endl;cout<<np<<endl;
    ll ans=0;
    if(abs(s1-s2)<=1)cout<<0<<endl;
    else {
        if(s1>s2){
            int rem=s1-(s2+1);
            rep(i,rem,0)ans+=p[i];
