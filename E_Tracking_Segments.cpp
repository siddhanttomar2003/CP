...
// pair<ll,bool> buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
//  if(l==r){
//    Seg_tree[i].first=v[l];
//    return Seg_tree[i];
//  }
//  ll mid=(l+r)/2;
//  pair<ll,bool> build_left=buildTree(2*i+1,l,mid,v,Seg_tree);
//  pair<ll,bool> build_right=buildTree(2*i+2,mid+1,r,v,Seg_tree);
//  Seg_tree[i].first=build_left+build_right;
//  return Seg_tree[i];
// }
//   pair<ll,bool> updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value,set<pair<int,int>>&st){
//        if(l==r &&r==a){
//         Seg_tree[i]=value;
//         return Seg_tree[i];
//        }
...
//       ll mid=(l+r)/2;
//       pair<ll,bool> left= updateTree(2*i+1,l,mid,a,Seg_tree,value);
//       pair<ll,bool> right= updateTree(2*i+2,mid+1,r,a,Seg_tree,value);
//       bool curr=false;
//       if(st.find({l,r})!=st.end() && left.first+right.first>(r-l+1)/2)curr=true;
//       return Seg_tree[i]={left.first+right.first,left.second|right.second|curr};
// }
 ...
 bool check(int &m, vi &queries,vpi &v, int n){
 ...
 void solve(){ 
    ...
    //   }
    //   rep(i,q,0)cout<<queries[i]<<" ";
    //   pe;
      int low=0;int high=q-1;
      int ans=high+1;
      while(low<=high){
         int mid=(low+high)/2;
        //  cout<<mid<<endl;
         if(check(mid,queries,v,n)){
             ans=mid;
             high=mid-1;
         }
         else {
