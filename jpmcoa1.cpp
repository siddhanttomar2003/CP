...
void solve(){
     ...
     int ans=INT_MAX;
     rep(i,n+1,0){
        int prev_zero=pre_count[i];
        int prev_one=abs(pre_count[i]-i);
        int rem=n-i+1;
        int ahead_zero=pre_count[n]-pre_count[i];
        int ahead_one=abs(rem-ahead_zero);
        ans=min(prev_one+ahead_zero,ans);
     }
