...
class Solution {
public:
    ll cal(vl &dp, vi &npower, ull &mp, int i){
        if(i==npower.size())return 0;
        if(dp[i]!=-1)return dp[i];
        int next_index=i+1;
        ll nottake=cal(dp,npower,mp,i+1);
        while(next_index<npower.size() && npower[next_index]-npower[i]<=2)next_index++;
        ll take=npower[i]*mp[npower[i]]+cal(dp,npower,mp,next_index);
    ...
    long long maximumTotalDamage(vector<int>& power) {
        ...
        vi npower;
        for(auto it:mp)npower.pb(it.first);
        sort(npower);
        vl dp(npower.size(),-1);
       return cal(dp,npower,mp,0);
