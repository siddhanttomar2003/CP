...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value){
  ...
  void dfs(vl &is_visited,vl &symbol, ll &count0,ll &count1,vvl &adj,ll curr,ll cs){
      is_visited[curr]=1;
      symbol[curr]=cs;
      if(cs==0)count0++;
      else count1++;
      for(int i=0;i<adj[curr].size();i++){
        if(!is_visited[adj[curr][i]]){
            dfs(is_visited,symbol,count0,count1,adj,adj[curr][i],cs^1);
        }
      }
  }
 void solve(){
...
vector<ll>is_visited(n+1,0);
vector<ll>symbol(n+1);
ll count0=0;ll count1=0;
for(int i=1;i<=n;i++){
    if(!is_visited[i]){
        dfs(is_visited,symbol,count0,count1,adj,i,0);
    }
}
ll ans=0;
for(ll i=1;i<=n;i++){
