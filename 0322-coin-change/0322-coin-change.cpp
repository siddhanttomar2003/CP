...
class Solution {
public:
    int recur(vi &coins, int amount , vvi &dp, int i){
        ...
        int take=1e8;
        if(coins[i]<=amount)take=1+recur(coins,amount-coins[i],dp,i);
        int nottake=1e8;
        nottake=recur(coins,amount,dp,i-1);
        return dp[i][amount]=min(take,nottake);
    }
    int coinChange(vector<int>& coins, int amount) {
        int n=coins.size();
        vvi dp(n,vi(amount+1,-1));
        int ans= recur(coins,amount,dp,n-1);
        if(ans>1e4)return -1;
        return ans;
    }
};