...
int cal(string &x, vi &b, int cost, int curr, int i){
    if(i==x.size()){
        return 0;
    }
    int mini=1e9;
    if(b[x[i]-65]){
        mini=min(mini,cal(x,b,cost,curr,i+1));
    }
    else {
        // hire instructor;
        mini=min(mini,curr+cal(x,b,cost,curr+1,i+1));
        b[x[i]-65]=1;
        mini=min(mini,cost+cal(x,b,cost,curr,i+1));
        b[x[i]-65]=0;
    }
    return mini;
}
