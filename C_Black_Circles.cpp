...
// vector<int>Seg_tree(4*n,0);
  ...
  long  double sx,sy;
   cin>>sx>>sy;
    long double tx,ty;
   cin>>tx>>ty;
  long double curr_dis=sqrt((tx-sx)*(tx-sx)+(ty-sy)*(ty-sy));
   rep(i,n,0){
    long double c_x=temp[i].first;
   long double c_y=temp[i].second;
    dis=min(dis,sqrt((tx-c_x)*(tx-c_x)+(ty-c_y)*(ty-c_y)));
   }
   if(dis<=curr_dis){
    pn;
   }
