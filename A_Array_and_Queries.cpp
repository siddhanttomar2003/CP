...
pair<ll, ll> buildTree(ll i, ll l, ll r, vector<ll> &v, vector<pair<ll, ll>> &Seg_tree)
{
    if (l == r)
    {
        Seg_tree[i].first = v[l];
        Seg_tree[i].second = l;
        return Seg_tree[i];
    }
    ll mid = (l + r) / 2;
    pair<ll, ll> build_left = buildTree(2 * i + 1, l, mid, v, Seg_tree);
    pair<ll, ll> build_right = buildTree(2 * i + 2, mid + 1, r, v, Seg_tree);
    Seg_tree[i].first = max(build_left.first, build_right.first);
    Seg_tree[i].second = (build_left.first > build_right.first) ? build_left.second : build_right.second;
    return Seg_tree[i];
}

void updateTree(ll i, ll l, ll r, ll a, vector<pair<ll, ll>> &Seg_tree, ll value)
{
    if (l == r && r == a)
    {
    ...
    ll mid = (l + r) / 2;
    updateTree(2 * i + 1, l, mid, a, Seg_tree, value);
    updateTree(2 * i + 2, mid + 1, r, a, Seg_tree, value);

    Seg_tree[i].first = max(Seg_tree[2 * i + 1].first, Seg_tree[2 * i + 2].first);
    Seg_tree[i].second = (Seg_tree[2 * i + 1].first > Seg_tree[2 * i + 2].first) ? Seg_tree[2 * i + 1].second : Seg_tree[2 * i + 2].second;
}

ll travel(ll i, ll a, ll b, ll l, ll r, ll x, vector<pair<ll, ll>> &Seg_tree, ll &ans)
{
    if (l > b || r < a)
    ...
    if (l >= a && r <= b)
    {
        if (Seg_tree[i].first <= x)
        {
            ans = min(ans, Seg_tree[i].second);
        }
    }
    ...
    ll mid = (l + r) / 2;
    travel(2 * i + 1, a, b, l, mid, x, Seg_tree, ans);
    travel(2 * i + 2, a, b, mid + 1, r, x, Seg_tree, ans);
    return ans;
}
...
void solve()
{
    ...
    vector<pair<ll, ll>> Seg_tree(4 * n, {0, 0});
    ...
    while (q--)
    {
        ...
        if (type == 1)
        {
            ll ind, val;
            cin >> ind >> val;
            updateTree(0, 0, n - 1, ind - 1, Seg_tree, val);
        }
        else
        {
            ll l, r, x;
            cin >> l >> r >> x;
            ll ans = 1e9;
            travel(0, l - 1, r - 1, 0, n - 1, x, Seg_tree, ans);
