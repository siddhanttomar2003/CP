...
class Solution {
public:
    int cal(vi &nums, int i, int k , int prev_index, vector<vvi> &dp){
        if(i==nums.size())return 0;
        if(dp[i][prev_index+1][k]!=-1)return dp[i][prev_index+1][k];
        int take=0;int  nottake=0;
        if(prev_index==-1){
            take=1+cal(nums,i+1,k,i,dp);
        }
        else if(nums[i]==nums[prev_index]){
            take=1+cal(nums,i+1,k,i,dp);
        }
        else if(k>0){
            take=1+cal(nums,i+1,k-1,i,dp);
        }
        nottake=cal(nums,i+1,k,prev_index,dp);
        return dp[i][prev_index+1][k]=max(take,nottake);
    }
    int maximumLength(vector<int>& nums, int k) {
        int n=nums.size();
        vector<vvi>dp(n,vvi(n+1,vi(k+1,-1)));
        int prev=0; int prev_index=-1;
        return cal(nums,0,k,prev_index,dp);
    }
};