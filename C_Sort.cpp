...
ll buildTree(ll i, ll l, ll r, string &x,string &y,vector<ll> &Seg_tree){
 if(l==r){
    if(x[l]!=y[l]) Seg_tree[i]=1;
    else Seg_tree[i]=0;
   return Seg_tree[i];
 }
 ll mid=(l+r)/2;
 ll build_left=buildTree(2*i+1,l,mid,x,y,Seg_tree);
 ll build_right=buildTree(2*i+2,mid+1,r,x,y,Seg_tree);
...
while(q--){
  inint(a);inint(b);
  int diff=0;
  vi temp1=fi[b];vi temp2=fi[a-1];
  vi temp3=si[b]; vi temp4=si[a-1];
...
//   pe;
  for(int i=0;i<26;i++){
    int diff1=0;int diff2=0;
   diff1+=(temp1[i]-temp3[i]);
   diff2+=(temp2[i]-temp4[i]);
   diff+=abs(diff1-diff2);
//    diff2+=abs(temp2[i]-temp4[i]);
  }
  cout<<diff/2<<endl;
}
// pe;
