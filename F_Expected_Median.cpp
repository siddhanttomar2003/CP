...
template <class T> ostream& operator<<(ostream& os, const vector<vector<T>>& vv) {for (vector<T>& v : vv) {os << v;} return os;}
...
int solve() {
    ...
    rep(i, n) if (a[i]) cnt++;
    mint ans;
    for (int i = (k + 1) / 2; i <= k; i++) if (i <= cnt) {
        ans += COM(cnt, i) * COM(n - cnt, k - i);
    }
    cout << ans.val() << endl;
    return 0;
}
