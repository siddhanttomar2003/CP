...
ll binomial_expo (ll a, ll b){
 ...
 int find_ways(int a, int b, vvi &dp){
    if(a==b)return 0;
    int count=1e8;
    if(dp[a][b]!=1e8)return dp[a][b];
    rep(i,a/2+1,1){
      count=min(count,1+find_ways(i,b,dp)+find_ways(a-i,b,dp));
    }
    rep(i,b/2+1,1){
        count=min(count,1+find_ways(a,i,dp)+find_ways(a,b-i,dp));
    }
    return dp[a][b]=count;
 }
...
int32_t main()
{
    ...
    //Code Karlo, Coz KHNH :)
     ...
     inint(a);
     inint(b);
     vvi dp(a+1,vi(b+1,1e8));
     int ans =find_ways(a,b,dp);
     cout<<ans<<endl;
