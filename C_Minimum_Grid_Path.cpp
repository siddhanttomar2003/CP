...
void solve(){
    ...
    rep(i,n,0){
      if(i>=2){
        if(i&1){
            ll rem=n-((i+1)/2)+1;
            ans=min(ans,curry+(v[i]*rem)+currx-lastx+(lastx*rem));
            ans=min(ans,curry+(v[i]*rem)+currx-last_minx+(last_minx*rem));
        }
        else {
            ll rem=n-((i)/2);
            ans=min(ans,currx+(v[i]*rem)+curry-lasty+(lasty*(rem+1)));
            ans=min(ans,currx+(v[i]*rem)+curry-last_miny+(last_miny*(rem+1)));
        }
      }
        if(i&1){
            curry+=v[i];
            lasty=v[i];
            last_miny=min(last_miny,v[i]);
      }
      else {
          currx+=v[i];
          lastx=v[i];
          last_minx=min(last_minx,v[i]);
      }
    }
    cout<<ans<<endl;
}
