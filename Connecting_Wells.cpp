 ...
 int orangesRotting(vector<vector<int>>& grid) {
        ...
        while(q.size()>0){
            ...
            for(int i=0;i<4;i++){
                int nr=sr+delRow[i];
                int nc=sc+delCol[i];
                if(isValid(nr,nc,n,m) && grid[nr][nc]==1 && vis.find({nr,nc})==vis.end()){
                    ans[nr][nc]=t+1;
                    vis[{nr,nc}]=1;
                    q.push({t+1,{nr,nc}});
                }
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                if(grid[i][j]==1 && ans[i][j]==INT_MAX)return -1;
                else if(grid[i][j]==1)maxi=max(ans[i][j],maxi);
