...
//    }
//    pe;
 std::  map<std::tuple<int,int,int>,ll>mp;
   for(int i=n-1;i>=n-3;i--){
    for(int j=n-1;j>=n-3;j--){
        for(int k=n-1;k>=n-3;k--){
            ll sum=0;
            sum+=(a[i].first+b[j].first+c[k].first);
            // mp[{a[i].second,b[j].second}]={c[k].second,sum};
            mp[std::make_tuple(a[i].second,b[j].second,c[k].second)]=sum;     
        }
    }
   }
   ll ans=INT_MIN;
   for(auto it:mp){
     auto key=it.first;
     ll one=std::get<0>(key);
     ll two=std::get<1>(key);
     ll third=std::get<2>(key);
     ll temp=it.second;
    // cout<<one<<","<<two<<","<<third<<"->"<<temp<<endl;
   if(one!=two && two!=third && third!=one){
