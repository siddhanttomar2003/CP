...
bool dfs(int start, vi &vis, vvi &adj, set<int>&st, int curr,int maxi){
    vis[start]=1;
    bool f=false;
    // cout<<start<<" "<<curr<<" ";
    pe;
    ...
    for(auto it:adj[start]){
      if(!vis[it]){
        if(it==n-1 && curr-1>=0)return true;
        else if(curr==0)return false;
        else {
            if(st.find(it)!=st.end())f|=dfs(it,vis,adj,st,maxi,maxi);
            else f|=dfs(it,vis,adj,st,curr-1,maxi);
        }
      }
    }
    return f;
}
bool check(int m , vvi &adj, set<int>&st){
    int n=adj.size();
    vi vis(n,0);
    return dfs(1,vis,adj,st,m,m);
}
void solve(){
    int n,m;
    cin>>n>>m;
    ...
    set<int>avail_pos(all(pos_e));
    int l=1,h=n;
    int ans=-1;
    while(l<=h){
        int m=l+(h-l)/2;
        // cout<<m<<" ->";
        if(check(m,adj,avail_pos)){
          ans=m;
          h=m-1;
        }
        else l=m+1;
