...
int32_t main()
{
    ...
    int start=v[0].first;
    int end=v[0].second;
    pq.push({end,curr_room});
     mp[{start,end}].pb(1);
     rep(i,n,1){
      if(pq.top().first>=v[i].first){
        curr_room++;
        mp[{v[i].first,v[i].second}].pb(curr_room);
        pq.push({v[i].second,curr_room});
      }
      else {
        mp[{v[i].first,v[i].second}].pb(pq.top().second);
         int room=pq.top().second;
         pq.pop();
         pq.push({v[i].second,room});
      }

     }
     cout<<pq.size()<<endl;
     rep(i,n,0){
        cout<<mp[{tempi[i].first,tempi[i].second}].back()<<" ";
         if(mp[{tempi[i].first,tempi[i].second}].size()>0)mp[{tempi[i].first,tempi[i].second}].pop_back();
     }
