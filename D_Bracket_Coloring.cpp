...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 int cal( vector<int> &temp, string x){
    stack<int>st;
    int ans;
     int counto=0;int countc=0;
    int n=x.length();
...
// vector<int>Seg_tree(4*n,0);
    ...
    string z=x;
    vector<int>temp1(n,0);vector<int>temp2(n,0);
    int ans1=cal(temp1,y);
    int ans2=cal(temp2,z);
    if(ans1<ans2 && ans1!= -1){
        cout<<ans1<<endl;
        rep(i,n,0)cout<<temp1[i]<<" ";
        pe;
    }
    else  if(ans2!= -1){
        cout<<ans2<<endl;
        for(int i=n-1;i>=0;i--)cout<<temp2[i]<<" ";  
        pe;
    }
    else cout<<-1<<endl;
