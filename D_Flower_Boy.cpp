...
void formcumulation(vector<ll>&c, vector<ll>&a, vector<ll>&b,int s){
    int j=0;
...
void solve(){
    ...
    inint(m);
    vl flower(n),c(m);
    rep(i,n,0)cin>>flower[i];
    rep(i,m,0)cin>>c[i];
    vector<ll>pre(n+1),suff(n+1);
    formcumulation(pre,flower,c,0);
    reverse(all(flower));
    reverse(all(c));
    formcumulation(suff,flower,c,1);
    reverse(all(suff));
    reverse(all(c));
    cout<<pre<<endl;
    cout<<suff<<endl;
