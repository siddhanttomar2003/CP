...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value){
  ...
  int dp[2001][2001];
  int cal(int i, int j , int k ,string &x, string &y, string &z){
    if(i==x.size() && j==y.size() && k==z.size()){
        return 0;
    }
    if(dp[i][j]!=-1)return dp[i][j];
    int tk1=1e9;
    if(i<x.size() && k<z.size() && j<y.size() && z[k]==x[i] && z[k]==y[j]){
        tk1=min(cal(i+1,j,k+1,x,y,z),cal(i,j+1,k+1,x,y,z));
    }
    int tk2=1e9;
    if(i<x.size() && k<z.size() && z[k]==x[i]){
        tk2=cal(i+1,j,k+1,x,y,z);
    }
    if(k<z.size() && j<y.size() && z[k]==y[j]){
        tk2=cal(i,j+1,k+1,x,y,z);
    }
    int tk3=1e9;
    if(i<x.size())tk3=1+cal(i+1,j,k+1,x,y,z);
    if(j<y.size())tk3=min(tk3,1+cal(i,j+1,k+1,x,y,z));
    return dp[i][j]=min({tk1,tk2,tk3});
