...
int findmax(int i, int j, int n, int m ,vector<string>&v){
    int l_i=i-1;
    int l_j=j-1;
    int r_i=i-1;
    int r_j=j+1;
    int left=0;int right=0;
    while(l_i>=0 && l_j>=0 && v[l_i][l_j]=='*'){
       left++;
       l_i--;l_j--;
    }
    while(r_i>=0 && r_j<m && v[r_i][r_j]=='*'){
        right++;
        r_i--;r_j++;
    }
    return min(left,right);
}
void ans(vector<string>&v,int i, int j, int n, int m, int req,vvi &dis){
    // max it can go for this *
    int avail= findmax(i,j,n,m,v);
    dis[i][j]=max(avail,dis[i][j]);
    bool consider=false;
    int l_i=i-1,l_j=j-1;
    int r_i=i-1,r_j=j+1;
    int left=avail,right=avail;
    while(left>0){
        dis[l_i][l_j]=max(dis[l_i][l_j],avail);
        l_i--;l_j--;
        left--;
    }
    while(right>0){
        dis[r_i][r_j]=max(dis[r_i][r_j],avail);
        r_i--;r_j++;
        right--;
    }
    return;
...
void solve(){
    ...
    for(int i=n-1;i>=0;i--){
        for(int j=m-1;j>=0;j--){
            if(v[i][j]=='*'){
              ans(v,i,j,n,m,req,dis);
            }
        }
    }
    rep(i,n,0){
        rep(j,m,0){
            if(v[i][j]=='*' && dis[i][j]<k)check=false;
        }
    }
    if(check)py;
    else pn;
