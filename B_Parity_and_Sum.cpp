...
vl v(n);inv;
int countodd=0;
ll largestodd=INT_MIN;
ll largesteven=INT_MIN;
int counteven=0;
rep(i,n,0){
    if(v[i]&1){
        countodd++;
        largestodd=max(largestodd,v[i]);
    }
    else {
        counteven++;
        largesteven=max(largesteven,v[i]);
    }
}
if(countodd>0){
        ...
        cout<<counteven<<endl;
         ...
         rep(i,n,0){
            if(v[i]%2==0){
                if(largestodd>v[i]){
                   v[i]+=largestodd;
                   largestodd=max(largestodd,v[i]);op++;
                }
            else flag=true;

            }
         }
         if(flag)cout<<counteven+1<<endl;
         else cout<<counteven<<endl;
