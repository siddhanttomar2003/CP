...
ll cal(vl &v1, vl &v2, int i, int turn, vvl &dp)
{
    if (i == v1.size())
    {
        return 0;
    ...
    if (turn == 0)
    {
        maxi1 = v1[i] + cal(v1, v2, i + 1, turn ^ 1, dp);
        maxi2 = cal(v1, v2, i + 1, turn, dp);
    }
    else {
        maxi1= v2[i]+cal(v1,v2,i+1, turn ^1 , dp);
        maxi2 = cal(v1,v2,i+1,turn , dp);
    }
    return dp[i][turn]= max(maxi1,maxi2);
}
void solve()
{
    ...
    cout << max(cal(v1, v2, 0, 0, dp), cal(v1, v2, 0, 1, dp));
}
