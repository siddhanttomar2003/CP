...
void spiralMatrixIII(int r, int c, int r_start, int c_start, vvi &ans){
      vector<pair<int,int>>dir={{0,1},{1,0},{0,-1},{-1,0}};
      int steps=1;
      int total=r*c;
      int curr_dir=0;
      int num=0;
      ans[r_start][c_start]=num;
      int times=1;
      while(times<total){
            int curr_steps=steps;
            while(curr_steps--){
                int n_r=r_start+dir[curr_dir].first;
                int n_c=c_start+dir[curr_dir].second;
...
void solve(){
     ...
     cin>>n;
     vvi ans(n,vi(n,0));
     spiralMatrixIII(n,n,(n/2+(n%2==0?-1:0)),(n/2+(n%2==0?-1:0)),ans);
     for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            cout<<ans[i][j]<<" ";
        }
