...
int32_t main()
{
    ...
    while(t--)
    {
     ...
     vi v1(n);
     inv1;
    vector<pair<ll,ll>>perperson(n);
    rep(i,n){
      perperson[i].first=v1[i];
      perperson[i].second=v[i];
    }
    sort(perperson.begin(),perperson.end());
    ll ans=0;
     int i=0;
    ...
    while(n>0){
       ...
       if(p<perperson[i].first){
        if(n>perperson[i].second)ans+=perperson[i].second*p;
        else ans+=n*p;
       }
       
        else{
if(n>perperson[i].second)ans+=perperson[i].second*perperson[i].first;
        else ans+=perperson[i].first*n;
   
        }
    n-=perperson[i].second;
    i++;
    }
   cout<<ans<<endl;
