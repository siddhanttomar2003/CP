...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 int dfs(int start,vvi &adj, vi &vis){
    vis[start]=1;
    int curr=1;
    for(int i=0;i<adj[start].size();i++){
        if(!vis[adj[start][i]]){
          curr+=dfs(adj[start][i],adj,vis);
        }
    }
    return curr;
 }
...
// vector<int>Seg_tree(4*n,0);
    ...
    //  }
     if(adj[start].size()==1){
        if((n-1)&1)cout<<"Ron"<<endl;
        else cout<<"Hermione"<<endl;
        return;
     }
     vector<int>vis(n+1,0);
     vis[start]=1;
     ll count_left=dfs(adj[start][0],adj,vis);
     ll count_right=dfs(adj[start][1],adj,vis);
