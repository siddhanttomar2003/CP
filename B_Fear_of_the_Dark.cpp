...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 double dis(int sx,int sy, int tx,int ty){
    return sqrt(pow(tx-sx,2)+pow(ty-sy,2));
 }
 void solve(){
// vector<int>Seg_tree(4*n,0);
double a,b,c,d,e,f;
cin>>a>>b>>c>>d>>e>>f;
double dis1=max(dis(0,0,c,d),dis(c,d,a,b));
double dis2=max(dis(0,0,e,f),dis(e,f,a,b));
double dis3=max({min(dis(0,0,c,d),dis(0,0,e,f)),min(dis(a,b,c,d),dis(a,b,e,f)),dis(c,d,e,f)/2});
cout<<setprecision(8)<<min({dis1,dis2,dis3})<<endl;
