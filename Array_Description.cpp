...
int find_ways(int i, vi &v, int m, int prev,vvi &dp,int n){
    if(i==n){
       return 1;
    }
    ...
    if(v[i]==0){
      if(prev!=1){
        count=(count+find_ways(i+1,v,m,prev-1,dp,n))%M;
      }
      if(prev!=m)count=(count+find_ways(i+1,v,m,prev+1,dp,n))%M;
      count=(count+find_ways(i+1,v,m,prev,dp,n))%M;
    }
    else {
     count+=  find_ways(i+1,v,m,v[i],dp,n);
    }
...
int32_t main()
{
  ...
  if(prev==0){
    rep(i,m+1,1){
       ans=ans+find_ways(1,v,m,i,dp,n);
    }
  }
  else {
    ans=find_ways(1,v,m,prev,dp,n);
  }
   cout<<ans<<endl;
    return 0;
}