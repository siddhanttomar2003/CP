...
int cal(vvl &dp, vl &v, int i, int prev, int m)
{
  if (i == v.size())
    return 1;
  if (dp[i][prev] != -1)
  ...
  if (v[i] != 0)
  {
    ...
    else ways = (ways + cal(dp,v,i+1,v[i],m))%M;
  }
  else
  { 
    if (prev - 1 >= 1)
    {
      ways =(ways+  cal(dp, v, i + 1, prev - 1,m))%M;
    }
    if (prev + 1 <= m)
    {
      ways  =(ways+ cal(dp, v, i + 1, prev + 1,m))%M;
    }
    ways = (ways+  cal(dp, v, i + 1, prev,m))%M;
...
void solve()
{
  ...
  vector<vector<ll>> dp(n, vector<ll>(102, -1));
  if(v[0]!=0){
  cal(dp, v, 1, v[0] , m);
  cout<<dp[1][v[0]];
  }
