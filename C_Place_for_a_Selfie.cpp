...
void solve(){
    ...
    inint(m);
    vector<int> lines(n);
    rep(i,n,0)cin>>lines[i];
    sort(lines);
    rep(i,m,0){
        double a , b , c; cin>>a>>b>>c;
        double num=4;
        double l=b-sqrt(num*a*c);
        double r=b+sqrt(num*a*c);
        int ind1=lower_bound(all(lines),r)-lines.begin();
        int ind2=upper_bound(all(lines),l)-lines.begin();
        //  cout<<r-1<<endl;
        //   cout<<ind1<<" "<<ind2<<endl;
        ind1--;
        if(ind1>=ind2){
