...
inint(n);
vector<string>v(2);
instr(x);v[0]=x;instr(y);v[1]=y;
// rep(i,2,0){
//     cout<<v[i]<<endl;
// }
int s= -1;
for(int i=0;i<n;i++){
    if(v[0][i]=='x' && v[1][i]=='x')s=i;
    else break;
}
int e=n;
for(int i=n-1;i>=0;i--){
    if(v[0][i]=='x' && v[1][i]=='x')e=i;
    else break;
}
int ans=0;
for(int i=0;i<n;i++){
    if(v[0][i]=='.' && v[1][i]=='.' && v[0][i-1]=='x' && v[0][i+1]=='x' && i-1>s && i+1<e){
        ans++;
    }
    else if(v[0][i]=='.' && v[1][i]=='.' && v[1][i-1]=='x' && v[1][i+1]=='x' && i-1>s && i+1<e)ans++;
}
cout<<ans<<endl;
