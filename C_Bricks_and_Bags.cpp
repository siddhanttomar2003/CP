...
// vector<int>Seg_tree(4*n,0);
 ...
 map<ll,ll>mp;
  ...
  ll ansi=v[n-1]-v[n-3]+v[n-2]-v[n-3];
  ll ansii=v[2]-v[1]+v[2]-v[0];
  n=temp.size();
  sort(temp);
  if(n>=3){
 ll maxi=temp[n-1];
 ll ans=(maxi-temp[0])+(maxi-temp[n-2]);
 ans=max(ans,maxi-temp[n-3]+temp[n-2]-temp[n-3]);
 ll ans2=(temp[1]-temp[0])+(temp[n-1]-temp[0]);
 ans2=max(ans2,temp[2]-temp[0]+temp[1]-temp[0]);
 cout<<max({ansi,ans,ans2,ansii})<<endl;
  }
  else {
    if(n==1)cout<<0<<endl;
    else {
        ll ans=0;
        ans=2*1ll*(temp[1]-temp[0]);
        cout<<max({ans,ansii,ansi})<<endl;
    }
  }
}
