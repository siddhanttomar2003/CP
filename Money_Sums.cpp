...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value){
  ...
  int cal(vl &v, int i, unordered_map<pl,ll> &dp,unordered_set<ll> &temp,ll sum){
    if(i<0){
        temp.insert(sum);
        return 0;
    }
    ...
    int t=0;int n_t=0;
    t=v[i]+cal(v,i-1,dp,temp,sum+v[i]);
    n_t=cal(v,i-1,dp,temp,sum);
  return  dp[{i,sum}]=t+n_t;
  }
 void solve(){
// vector<int>Seg_tree(4*n,0);
 inint(n);
  ...
  temp.erase(0);
  cout<<temp.size()<<endl;
//   rep(i,temp.size(),0)cout<<temp[i]<<" ";
vl vec;
for(auto it:temp){
   vec.pb(it);
}
sort(vec);
rep(i,vec.size(),0)cout<<vec[i]<<" ";
pe;
}
