...
int32_t main()
{
    ...
    vvi vv(n,vi(m));
   invv;
     // if neg are even then all will become +;
     // if neg are odd then if there is one 0 then it will again become all +;
     // else one neg will be left always
     int count=0;
     ll sum=0;
     int mini=INT_MAX;
     bool flag=false;
     rep(i,n){
        rep(j,m){
            ...
            mini=min(mini,abs(vv[i][j]));
            if(vv[i][j]==0)flag=true;
