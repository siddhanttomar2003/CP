...
// to count how many balls will collide
 inint(n);
 vector< long double>pos(n);
 vector< long double>sp(n);
 rep(i,n,0){
     cin>>pos[i];
 }
 rep(i,n,0){
 ...
 rep(i,n,0){
    ti[i]=v[i].first/v[i].second;
 }
 ll ans=0;
 // posn of all less time 
 vector<pair< long double, long double>>temp;
 rep(i,n,0){
    temp.push_back({ti[i],i+1});
 }
 sort(temp);
 int till= -1;
 int count=0;
 rep(i,n,0){
    int curr_pos=temp[i].second;
    if(curr_pos>till){
        till=curr_pos;
        count++;
    }
