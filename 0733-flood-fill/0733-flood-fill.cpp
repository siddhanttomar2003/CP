...
public:
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int color) {
        ...
        //   mp[{start_i,start_j}]++;
          q.pop();
          // left
          if(start_j-1>=0 && image[start_i][start_j-1]==curr && mp.find({start_i,start_j-1})==mp.end()){
             mp[{start_i,start_j-1}]++;
             image[start_i][start_j-1]=color;
             q.push({start_i,start_j-1});
          }
          //right
             if(start_j+1<m && image[start_i][start_j+1]==curr && mp.find({start_i,start_j+1})==mp.end()){
             mp[{start_i,start_j+1}]++;
             image[start_i][start_j+1]=color;
             q.push({start_i,start_j+1});
          }
          if(start_i-1>=0 && image[start_i-1][start_j]==curr && mp.find({start_i-1,start_j})==mp.end()){
          mp[{start_i-1,start_j}]++;
          image[start_i-1][start_j]=color;
          q.push({start_i-1,start_j});
          }
             if(start_i+1<n && image[start_i+1][start_j]==curr && mp.find({start_i+1,start_j})==mp.end()){
          mp[{start_i+1,start_j}]++;
          image[start_i+1][start_j]=color;
          q.push({start_i+1,start_j});
          }
