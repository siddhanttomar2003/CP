...
//  cout<<tr<<" "<<tc<<endl;
 ...
 q.push({0,{sr,sc}});
 vector<vector<int>>dis(n,vi(m,1e9));
 ...
 while(q.size()>0){
    ...
    for(int k=0;k<4;k++){
        int nr=i+delRow[k];
        int nc=j+delCol[k];
        if(nr>=0 && nc<m && nc>=0 && nr<n && v[nr][nc]=='.' || v[nr][nc]=='B' && dis[nr][nc]>wt+1 ){
         mp[{nr,nc}]={i,j};
         dis[nr][nc]=wt+1;
         if(v[nr][nc]=='B')continue;
         v[nr][nc]='#';
         q.push({wt+1,{nr,nc}});
        }     
    }
 }
 if(mp.find({tr,tc})!=mp.end()){
    py;
    string ans="";
    while(mp[{tr,tc}]!=make_pair(tr,tc)){
        int curr_r=tr;int curr_c=tc;
        int prev_r=mp[{tr,tc}].first;
        int prev_c=mp[{tr,tc}].second;
        int delr=curr_r-prev_r;
        if(delr== -1)ans+='U';
        else if(delr== 1)ans+='D';
        int delc=curr_c-prev_c;
        if(delc == -1){
            ans+='L';
        }
        else if(delc == 1){
