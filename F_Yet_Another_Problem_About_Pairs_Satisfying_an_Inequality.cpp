...
void solve() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (v[i] < i + 1) {
            auto it = std::upper_bound(sortedVec.begin(), sortedVec.end(), i + 1);
            int count = sortedVec.end() - it;  // Manually calculate the distance
            ans += count;
            sortedVec.push_back(v[i]);
        }
        // sortedVec.insert(std::upper_bound(sortedVec.begin(), sortedVec.end(), v[i]), v[i]);
    }
