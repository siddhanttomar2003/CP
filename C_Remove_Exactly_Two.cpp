...
void solve(){
   ...
   map<ll,ll>mp;
   rep(i,n+1,1){
    maxi=max(maxi,(ll)adj[i].size());
    mp[adj[i].size()]++;
   }
   vector<ll>sus;
   rep(i,n+1,1){
    if(adj[i].size()==maxi)sus.pb(i);
   }
   ll ans=0;
   for(auto it:sus){
     mp[adj[it].size()]--;
     if(mp[adj[it].size()]==0)mp.erase(adj[it].size());
     for(auto it2:adj[it]){
         ans=max(ans,(ll)adj[it].size()+(ll)adj[it2].size()-2);
         mp[adj[it2].size()]--;
         if(mp[adj[it2].size()]==0)mp.erase(adj[it2].size());
     }
     if(mp.size()>0){
