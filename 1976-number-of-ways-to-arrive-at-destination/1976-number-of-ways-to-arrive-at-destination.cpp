    ...
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        while(pq.size()>0){
            ...
            rep(i,adj[par].size(),0){
                ll child=adj[par][i].first;
                ll child_time=adj[par][i].second;
                if(dis[child]==ti+child_time){
                    dp[child]=(dp[par]+dp[child])%M;
                }
                else if(dis[child]>ti+child_time){
                    dis[child]=ti+child_time;
                    dp[child]=dp[par];
