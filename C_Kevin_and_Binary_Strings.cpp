...
string calxor(string t1,string t2){
    int j=t1.size()-1;
    int i=t2.size()-1;
    while(i>=0 && j>=0){
        if(t1[j]==t2[i])t2[i]='0';
...
void solve(){
    ...
    int s2=n;int e2=n;
    if(pos!=-1){
      string tar=x.substr(pos,n-pos);
    //   cout<<tar<<endl;
      map<string,pair<int,int>>mp;
      int s=tar.size();
      for(int i=0;i<=n-s;i++){
        string temp=x.substr(i,s);
        mp[temp]={i,i+s-1};
      }
      string ans=x;
      for(auto it:mp){
          string curr= calxor(it.first,x);
          if(curr>ans){
                ans=curr;
                s2=it.second.first+1;
                e2=it.second.second+1;
          }
      }
    }
