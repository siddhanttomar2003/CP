...
int32_t main()
{
    ...
    //Code Karlo, Coz KHNH :)
     ...
     inv;
     sort(v.begin(),v.end());
     long long max_diff=v[n-1]-v[0];
     long long count_max=1;
     long long count_min=1;
     int start=0;
     while(start<n-1){
        if(v[start]==v[start+1])count_min++;
        else break;
        start++;
     }
     int end=n-1;
     while(end>0){
        if(v[end]==v[end-1])count_max++;
        else break;
        end--;
     }
     if(max_diff==0){
        if(n==2)cout<<max_diff<<" "<<"1"<<endl;
        else cout<<max_diff<<" "<<(n*(n-1))/2<<endl;
     }
     else {
        long long ans=count_min*count_max;
        cout<<max_diff<<" "<<ans<<endl;
     }
