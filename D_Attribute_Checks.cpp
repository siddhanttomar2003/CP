...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 int cal(int i , vvi &dp, vl &v, int c_a, int c_b){
    if(i==v.size()){
        return 0;
    }
    if(dp[c_a][c_b]!=-1)return dp[c_a][c_b];
        int w1=0;int w2=0;
    if(v[i]==0){
        w1=cal(i+1,dp,v,c_a+1,c_b);
        w2=cal(i+1,dp,v,c_a,c_b+1);
    }
    else {
        if(v[i]>0){
            if(c_a>v[i])
            w1=1+cal(i+1,dp,v,c_a,c_b);
            else  w1=cal(i+1,dp,v,c_a,c_b);
        }
        else {
            if(c_b>abs(v[i])){
                w1=1+cal(i+1,dp,v,c_a,c_b);
                ...
                w1=cal(i+1,dp,v,c_a,c_b);
            
        }
        
    }
    return dp[c_a][c_b]=max(w1,w2);
