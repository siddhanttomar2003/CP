...
void precomp(ll n,ll p){
    ...
    pre_fac[0]=1;
    ll i=0;
    for(i=1;i<n;i++)  pre_fac[i]=(pre_fac[i-1]*i)%p;
    i--;
    invfact[i]=mpow(pre_fac[i],p-2,p);
    for(i--;i>=0;i--){
...
ll C(ll n,ll r,ll p){
    ...
    if(r==0) return 1;
    return pre_fac[n] * invfact[r] % p * invfact[n-r] %p;
}
...
void solve(){
    ...
    if(diff>1)cout<<0<<endl;
        ...
        rep(i,n,0)mp[v[i]]++;
        ll second_max=v[n-2];
        ll freq=mp[second_max];
        ll rem=n-freq-1;
        cout<<((pre_fac[n]-mul(C(n,count+1,mod),mul((pre_fac[n-count-1]),pre_fac[count])))%M+M)%M<<"\n";
        cout<<ansi<<endl;
    }
}
