...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll cal(map<pair<ll,ll>,ll>&dp , vpl &temp, ll curr, int i){
    if(i==temp.size()){
        return 0;
    }
    if(curr<temp[i].first)return 0;
   if(dp.find({i,curr})!=dp.end())return dp[{i,curr}];
    ll w1=0;ll w2=0;
    if(temp[i].first==curr){
       w1=(temp[i].second-1)+cal(dp,temp,curr+temp[i].second-1,i+1);
    }
    
    w2=cal(dp,temp,curr,i+1);
    return dp[{i,curr}]=max(w1,w2);
 }
 void solve(){
...
//  pe;
 ...
 cout<<n+cal(dp,temp,curr,0)<<endl;
