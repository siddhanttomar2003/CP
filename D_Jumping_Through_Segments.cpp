...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value){
  ...
  bool check(vpi &v, int len){
    int curr_s=0;int curr_e=0;
     for(int i=0;i<v.size();i++){
       if(curr_e+len<v[i].first)return false;
       if(curr_s-len>v[i].second)return false;
       else {
        curr_e=min(v[i].second,curr_e+len);
        curr_s=max(v[i].first,curr_s-len);
       }
     }
     return true;
  }
