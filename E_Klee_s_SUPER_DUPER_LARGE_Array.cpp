...
bool check(ll curr, ll start, ll end)
{
    ll s1 = (curr * (curr + 1)) / 2 - ((start - 1) * (start)) / 2;
    ll s2 = (end * (end + 1)) / 2 - (curr * (curr + 1)) / 2;
    if (s1 >= s2)
        return true;
    return false;
}
void solve()
{
 ...
 ll ans1=abs((pos*(pos+1)/2 - ((start-1)*(start))/2) - ((end*(end+1))/2 - (pos*(pos+1))/2));
 
  ll ans2=1e10;
  if(pos-1>=start){
    pos-=1;
  ans2=abs((pos*(pos+1)/2 - ((start-1)*(start))/2) - ((end*(end+1))/2 - (pos*(pos+1))/2));
  }
