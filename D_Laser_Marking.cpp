...
// vector<int>Seg_tree(4*n,0);
   ...
   long double s,t;cin>>s>>t;
    map<int,vector<pair<long double,long double>>>mp;
    rep(i,n,0){
        long double a,b,c,d;cin>>a>>b>>c>>d;
        mp[i].pb({a,b});mp[i].pb({c,d});
    }
    // for(auto it:mp){
    //     cout<<it.first<<endl;
    //     cout<<it.second[0].first<<" "<<it.second[0].second<<" "<<it.second[1].first<<" "<<it.second[1].second<<endl;
    // }
    long double ans=0;
      long double cx=0;long double cy=0;
    while(mp.size()>0){
        long double c_dis=1e8;
        long double tt=0;
        ll ind=-1;
        long double ex=0;ll ey=0;
        for(auto it:mp){
            pair<long double, long double>p1=it.second[0];
            pair<long double, long double>p2=it.second[1];
            long double dis1=sqrtl(((p1.first-cx)*(p1.first-cx))+((p1.second-cy)*(p1.second-cy)));
            long double dis2=sqrtl(((p2.first-cx)*(p2.first-cx))+((p2.second-cy)*(p2.second-cy)));
            if(dis1<c_dis){
                c_dis=dis1;
                ind=it.first;
                ex=p2.first;
                ey=p2.second;
                tt=sqrtl(((p2.first-p1.first)*(p2.first-p1.first))+((p2.second-p1.second)*(p2.second-p1.second)));
            }
            if(dis2<c_dis){
                c_dis=dis2;
                ind=it.first;
