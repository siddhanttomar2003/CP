...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 ll updateTree(ll i,ll l ,ll r , ll a, vl &Seg_tree,ll value){
  ...
  ll travel(ll i, ll a, ll b, ll l , ll r , vl &Seg_tree ){
   ...
   bool check(int mid, vector<vector<int>>&bitm, int k , int s){
    string ans="";
       for(int i=32;i>=0;i--){
         if(bitm[mid][32-i-1]-bitm[s-1][32-i-1]==mid-s+1){
            ans+='1';
         }
         else ans+='0';
    }
    ll num=binaryToDecimal(ans);
...
// vector<int>Seg_tree(4*n,0);
  inint(n);vl v(n);inv;
  vector<vector<int>>bitm(n+1,vector<int>(32,0));
