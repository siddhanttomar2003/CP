...
int32_t main()
{
    ...
    ll n,k,q;
    cin>>n>>k>>q;
     vector< long double>dis(k);
     vector< long double>time(k);
     rep(i,k)cin>>dis[i];
     rep(i,k)cin>>time[i];
    // vector< long double>speed(k);
    // rep(i,k){
    //     if(i==0)speed[i]=((dis[i]*1.0)/(time[i]*1.0));
    //     else speed[i]=(((dis[i]-dis[i-1])*1.0)/((time[i]-time[i-1])*1.0));
    // }
     while(q--){
        inll(d);
        auto it =lower_bound(dis.begin(),dis.end(),d);
       int i=it-dis.begin();
              ll min=0;   
                if(dis[i]==d)min=time[i];
              else   if(i==0){
                    min=floor((d*time[0])/dis[0]);                  
                }
              
                else {
                    min=floor(time[i-1]+(((d-dis[i-1])*(time[i]-time[i-1]))/(dis[i]-dis[i-1])));
                } 
        cout<<min<<" ";
     }
     pe;
