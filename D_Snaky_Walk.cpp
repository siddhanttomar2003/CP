...
void solve()
{
    ...
    pair<int, int> start;
    pair<int, int> end;
    map<pair<int, int>, array<ll, 2>> dis;  // Store distances for vertical (0) and horizontal (1)
    ...
    rep(i, n, 0)
    {
        rep(j, m, 0)
        {
            ...
            dis[{i, j}][0] = dis[{i, j}][1] = 1e18;  // Initialize distances to infinity
        }
    }

    dis[start][0] = dis[start][1] = 0;  // Start with distance 0 for both move types

    priority_queue<pair<pair<ll, ll>, pair<int, int>>, vector<pair<pair<ll, ll>, pair<int, int>>>, greater<pair<pair<ll, ll>, pair<int, int>>>> pq;
    pq.push({{0, -1}, start});  // Initial distance is 0, no previous move type
    ...
    while (!pq.empty())
    {
        ll curr_dis = pq.top().first.first;
        ll prev_move_type = pq.top().first.second;
        ...
        for (int i = 0; i < 4; i++)
        {
            int new_r = curr_r + delRow[i];
            int new_c = curr_c + delCol[i];
            int move_type = (abs(new_r - curr_r) == 1) ? 0 : 1;  // 0 for vertical, 1 for horizontal

            if (check(new_r, new_c, n, m) && v[new_r][new_c] != '#')
            {
                if (move_type != prev_move_type && dis[{new_r, new_c}][move_type] > curr_dis + 1)
                {
                    if (v[new_r][new_c] == 'G')
                    {
                        cout << curr_dis + 1 << endl;
                        return;
                    }
                    dis[{new_r, new_c}][move_type] = curr_dis + 1;
                    pq.push({{curr_dis + 1, move_type}, {new_r, new_c}});
                }
            }
        }
    }
