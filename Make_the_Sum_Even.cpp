...
int32_t main()
{
    ...
    while(t--)
    {
     ...
     if(co%2==0)cout<<0<<endl;
     else {
         // now we have to make one odd ->even  we cannot convert odd->even by changing the  no. by its any power 
         // or we can make      one even ->odd;
         bool check=false;
         rep(i,n,0){
            if(v[i]%2==0){
                int power=v[i]/2-1;
                if(power==0)check=true;
            }
         }
