...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 int dfs(int start, vvi &adj, vi &l_nodes,vi &visited){
    visited[start]=1;
     int curr=0;
     bool check=true;
     rep(i,adj[start].size(),0){
        if(!visited[adj[start][i]]){
            check=false;
            curr+=dfs(adj[start][i],adj,l_nodes,visited);
        }
     }
     
     if(check)return l_nodes[start]=1+curr;
     return l_nodes[start]= curr;
 }
 void solve(){
// vector<int>Seg_tree(4*n,0);
 ...
 rep(i,n-1,0){
  ...
  inint(q);
 vector<int>leave_nodes(n+1,0);
 vector<int>visited(n+1,0);
 dfs(1,adj,leave_nodes,visited);
//  rep(i,n+1,1)cout<<leave_nodes[i]<<" ";
//  pe;
  while(q--){
    inint(a);
    inint(b);
    ll num1=leave_nodes[a];
    ll num2=leave_nodes[b];
    cout<<num1*num2<<endl;
  }
}
