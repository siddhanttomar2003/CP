...
void solve()
{
    ...
    map<int, vector<pair<int, char>>> mp_c, mp_r;
    bool check = true;
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        char c;
        cin >> row >> col >> c;
        mp_c[col].push_back({row, c});
        mp_r[row].push_back({col, c});
    }

    for (auto it : mp_c)
    {
        vector<pair<int, char>> temp = it.second;
        sort(temp); // Sort by row
        int pos_lastwhite = -1;
        int pos_firstblack = -1;
        ...
        for (auto p : temp)
        {
            ...
            if (c == 'W')
                pos_lastwhite = pos;
            else if (pos_firstblack == -1)
                pos_firstblack = pos;
        }

        if (pos_lastwhite != -1 && pos_firstblack != -1 && pos_lastwhite < pos_firstblack)
        {
            check = false;
            break;
        }
        else {
            rep(j,pos_lastwhite,0){
                mp_r[j].pb({it.first,'B'});
            }
        }
    }
