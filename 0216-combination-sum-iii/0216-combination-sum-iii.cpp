...
class Solution {
public:
    void recur(vvi &ans,vi &temp,vi &can,int i,int lim, int sum){
        if(temp.size()>lim)return;
        if(i==can.size())return;
        if(sum==0 && temp.size()==lim){
            ans.pb(temp);
            return;
        }
        if(can[i]<=sum){
            temp.pb(can[i]);
            recur(ans,temp,can,i+1,lim,sum-can[i]);
            temp.pop_back();
        }
        recur(ans,temp,can,i+1,lim,sum);
    }
