...
int32_t main()
{
    ...
    cin>>sx>>sy>>ex>>ey;
    instr(x);
    int req_x_pos=0;int req_x_neg=0;
    if(ex>sx)req_x_pos=ex-sx;
    else req_x_neg=sx-ex;
    int req_y_pos=0;int req_y_neg=0;
    if(ey>sy)req_y_pos=ey-sy;
    else req_y_neg=sy-ey;
    int actual_x_pos=0;int actual_x_neg=0;
    int actual_y_pos=0;int actual_y_neg=0;
    int time=0;
    bool flag=false;
    rep(i,t){
        if(x[i]=='S')actual_y_neg++;
        else if(x[i]=='N')actual_y_pos++;
        else if(x[i]=='E')actual_x_pos++;
        else actual_x_neg++;
        time++;
        if(actual_x_pos>=req_x_pos&&actual_x_neg>=req_x_neg&&actual_y_pos>=req_y_pos&&actual_y_neg>=req_y_neg){
            flag=true;break;
        }
