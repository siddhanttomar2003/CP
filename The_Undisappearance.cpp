...
// vector<int>Seg_tree(4*n,0);
  ...
  if(c1>1)ans++;
    ...
    for(int i=2;i<=n;i++){
        temp[i][0]=v[i-1]==1?temp[i-1][0]+1:temp[i-1][0];
        temp[i][1]=v[i-1]==2?temp[i-1][1]+1:temp[i-1][1];
        temp[i][2]=v[i-1]==3?temp[i-1][2]+1:temp[i-1][2];
    }
    // for(int i=0;i<=n;i++){
   ...
   for(int i=n-1;i>0;i--){
                  ...
                  while(l<=h){
                    // if(count>=2)break;
                    int mid=l+(h-l)/2;
                    int cal1=temp[mid][0]-temp[i-1][0];
                    int cal2=temp[mid][1]-temp[i-1][1];
                    int cal3=temp[mid][2]-temp[i-1][2];
                    // cout<<mid<<" ";cout<<s-1<<" ";
                    // count++;
                    if(c1-cal1>0 && c2-cal2>0 && c3-cal3>0){
                         curr=max(curr,mid);
                         l=mid+1;
                    }
                    else {
    ...
    //   ans++;
    //  }
    //  else if(v[0]==2 && c2>1)ans++;
    //  else if(v[0]==3 && c3>1)ans++; 
   cout<<ans<<endl;
