...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        // so clearly if there exists any topological sort order
        // then we can follow an order to pick all the courses
        // we can say that if there exist any cycle then we cannot find a //
        // topological sort order
        vector<vector<int>> adj(numCourses);
        rep(i, prerequisites.size(), 0) {
