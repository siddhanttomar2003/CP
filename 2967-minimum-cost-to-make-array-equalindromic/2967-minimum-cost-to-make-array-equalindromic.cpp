   ...
   ll cal(vi &nums, ll num){
    ...
    long long minimumCost(vector<int>& nums) {
        ...
        if(n&1){
            ...
            m=(nums[n/2]*1ll+nums[n/2-1]*1ll)/(2*1ll);   
        }
         ll left_closest=m-1;
         while(!(isPal(left_closest))){
            left_closest--;
         }
         ll right_closest=m+1;
         while(!(isPal(right_closest))){
            right_closest++;
         }
         ll ans1=cal(nums,left_closest);
         ll ans2=cal(nums,right_closest);
         ll ans3=1e15;
         if(isPal(m)){
               ans3=cal(nums,m);
         }
         return min(ans1,min(ans3,ans2));
