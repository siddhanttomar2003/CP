...
void solve(){
    ...
    rep(i,n,0)
    {
        inll(s);
        inll(a);
        if(s==1)maxi_1=max(maxi_1,a);
        else maxi_2=max(maxi_2,a);
    }
    ll ans=1e18;
    if(maxi_1!=0){
        ans=min(ans,d/maxi_1+(d%maxi_1==0?0:1));
    }
     if(maxi_2!=0){
        if(d%maxi_2==0){
            ans=min(ans,(d/maxi_2)*2*1ll);
        }
        else {
            ll rem=d%maxi_2;
            if(maxi_1>=rem){
                ans=min(ans,(d/maxi_2)*2*1ll+1*1ll);
            }
            else {
                ans=min(ans,(d/maxi_2+1)*2*1ll);
            }
        }
     }
     cout<<ans<<endl;
