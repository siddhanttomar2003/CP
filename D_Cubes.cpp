...
long long findCubeRoot(long long num) {
    long long low = 1, high = 1000002;
    while (low <= high) {
        long long mid = low + (high - low) / 2;
        long long midCube = mid * mid * mid;
        if (midCube == num)
            return mid;
        if (midCube < num)
            low = mid + 1;
...
void solve()
{
    ...
    for (long long a = 1; a <= 1000001; a++) {
        long long aCube = a * a * a;
        if (aCube <= target) continue;
        
        long long diff = aCube - target;
        if (diff <= 0) continue;
        
        long long b = findCubeRoot(diff);
        if (b > 0 && b * b * b == diff && a==target+b) {
            cout << a << " " << b << "\n";
            return ;
        }
    }
