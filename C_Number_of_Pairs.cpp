...
// vector<int>Seg_tree(4*n,0);
 ...
 for(int i=0;i<n-1;i++){
    int lo=l-v[i];
    int hi=r-v[i];
    auto it1=lower_bound(v.begin(),v.end(),lo);
    auto it3=upper_bound(v.begin(),v.end(),hi);
    int it=lower_bound(v.begin(),v.end(),lo)-v.begin();
    if(it1==v.end())continue;
    it=max(it,i+1);
    int it2=upper_bound(v.begin(),v.end(),hi)-v.begin();
    it2--;
    if(it3==v.begin())continue;
      if(it2>=it)
    pairs+=(it2-it+1);
 }
 cout<<pairs<<endl;
