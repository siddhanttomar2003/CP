...
void get_primes(int n){ for(int i = 2; i <= n; i++)  if(is_prime[i])  primes.push_back(i); }
 void dfs(int start, vi &vis, vi &ans,vvi &adj, int tar, int val){
    vis[start]=1;
    ans[start]=val;
    rep(i,adj[start].size(),0){
        int child=adj[start][i];
        if(!vis[child] && child!=tar){
            dfs(child,vis,ans,adj,tar,val^1);
        }
    }
 }
void solve(){
    ...
    if(abs(x-y)==1 || (x==1 && y==n)){
        int last_eve=(n%2==0?n:n-1);
        rep(i,last_eve,0)cout<<i%2<<" ";
        if(last_eve!=n)cout<<2<<" ";
        pe;
    }
    else {
       vi vis(n+1,0);
       vi ans(n+1,-1);
       dfs(x,vis,ans,adj,y,0);
    //    cout<<ans<<endl;
       set<int>st;
       int left=(y==1?n:y-1);
       int right=(y==n?1:y+1);
       st.insert(ans[left]);st.insert(ans[right]);
       st.insert(ans[x]);
       int curr=0;
       for(auto it:st){
        if(it!=curr){
