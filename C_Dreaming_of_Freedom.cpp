...
void Sieve(int n){ is_prime.assign(n + 1, 0); is_prime[0] = is_prime[1] = 0; for(ll i = 2; i * i <= n; i++) if(is_prime[i]) for(ll j = i * i; j <= n; j += i) is_prime[j] = false;}
