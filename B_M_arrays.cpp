...
void solve()
{
   ...
   auto it = mp.begin();
    while (it != mp.end()) {
        ...
        auto reqIt = mp.find(req);
        if (reqIt != mp.end()) {
            if (reqIt->second < it->second) {
                it->second -= (reqIt->second + 1);
                if (it->second > 0) count += it->second;
            } else if (reqIt->second > it->second) {
                reqIt->second -= (it->second + 1);
                if (reqIt->second > 0) count += reqIt->second;
            }
            count++;
            mp.erase(reqIt);
            it = mp.erase(it);
