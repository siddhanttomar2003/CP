...
ll buildTree(ll i, ll l, ll r, vector<ll> & v,vector<ll> &Seg_tree){
 ...
 void dfs(vvi &adj, int s, int curr, vi &is_visited,int &ans,map<int,char>&mp){
    is_visited[s]=1;
    if(adj[s][0]==-1 && adj[s][1]==-1){
        ans=min(ans,curr);
        return;
    }
    if(adj[s][0]!=-1){
        if(!is_visited[adj[s][0]]){
        if(mp[s]!='L'){
            dfs(adj,adj[s][0],curr+1,is_visited,ans,mp);
        }
        else {
            dfs(adj,adj[s][0],curr,is_visited,ans,mp);
        }
        }
    }
    if(adj[s][1]!=-1){
        if(!is_visited[adj[s][1]]){
          if(mp[s]!='R'){
            dfs(adj,adj[s][1],curr+1,is_visited,ans,mp);
        }
        else {
            dfs(adj,adj[s][1],curr,is_visited,ans,mp);
        }
        }
    }
 }
...
//    cout<<i+1<<" ->"<<adj[i+1][0]<<" "<<adj[i+1][1]<<endl;
 }

 int ans=INT_MAX;
 vector<int>is_visited(n+1,0);
  dfs(adj,1,0,is_visited,ans,mp);
  cout<<ans<<endl;
}
