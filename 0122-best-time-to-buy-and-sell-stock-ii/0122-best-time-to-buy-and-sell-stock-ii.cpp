...
class Solution {
public:
     int cal(int i, vi &prices, vvi &dp, int status){
        if(i==prices.size()){
            return 0;
        }
        ...
        int t=0;int nt=0;
        if(status==0){
           t=(-prices[i])+cal(i+1,prices, dp ,1);
           nt=cal(i+1,prices,dp,0);
        }
        else {
            t=prices[i]+cal(i+1,prices,dp,0);
            nt=cal(i+1,prices,dp,1);
        }
        return dp[i][status]=max(t,nt);
     }
    int maxProfit(vector<int>& prices) {
        int n=prices.size();
        vvi dp(n,vi(2,-1));
      return  cal(0,prices,dp,0);
    }
};