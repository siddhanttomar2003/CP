...
ll recur(vl &v,vvl &dp, int i, ll target){
    ...
    if(v[i]<=target)take=recur(v,dp,i,target-v[i]);
   nottake=recur(v,dp,i-1,target);
...
int32_t main()
{
    ...
    //Code Karlo, Coz KHNH :)
     ...
     inll(x);
     vi coins(n);
     rep(i,n,0)cin>>coins[i];
     vector<vector<int>> dp(n + 1, vector<int>(x + 1, 0));
    dp[0][0] = 1; // There's one way to make sum 0 with 0 coins.

    for (int i = 1; i <= n; ++i) {
        for (int sum = 0; sum <= x; ++sum) {
            // Number of ways to make 'sum' without using the current coin
            dp[i][sum] = dp[i - 1][sum];
            
            // Number of ways to make 'sum' including the current coin
            if (sum >= coins[i - 1]) {
                dp[i][sum] = (dp[i][sum] + dp[i][sum - coins[i - 1]]) % M;
            }
        }
    }
