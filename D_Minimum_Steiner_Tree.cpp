...
bool dfs(int start,vvl &adj, vl &vis, set<int>&ans,set<int>&req){
    vis[start]=1;
    bool flag=false;
    if(req.find(start)!=req.end()){
        flag=true;
    }
    for(int i=0;i<adj[start].size();i++){
        int child=adj[start][i];
       if(!vis[child]){
          flag|=dfs(child,adj,vis,ans,req);
       }
    }
    if(flag)ans.insert(start);
    return flag;
}
void solve()
{
