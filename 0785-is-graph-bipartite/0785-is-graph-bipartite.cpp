...
class Solution {
public:
       void dfs(int curr, vi &is_visited, vi &temp, vvi &graph, int flip){
        is_visited[curr]=1;
        temp[curr]=flip;
        flip^=1;
        rep(i,graph[curr].size(),0){
            if(!is_visited[graph[curr][i]]){
                dfs(graph[curr][i],is_visited,temp,graph,flip);
    ...
    bool isBipartite(vector<vector<int>>& graph) {
        ...
        vi is_visited(n,0);
        vi temp(n);
         dfs(0,is_visited,temp,graph,0);
         rep(i,n,0){
            int ff=temp[i];
            rep(j,graph[i].size(),0){
                if(ff==temp[graph[i][j]])return false;
            }
         }
         return true;
    }
