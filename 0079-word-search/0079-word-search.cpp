...
class Solution {
public:
    void dfs(vvi& is_visited, vector<vector<char>>& board, string word, int i,
             bool &temp, int start_r, int start_c) {
        if (i == word.size()) {
            temp = true;
            return;
        }
        ...
        int m = board[0].size();
        rep(j, 4, 0) {
            int new_r = start_r + delRow[j];
            int new_c = start_c + delCol[j];
            if (new_r >= 0 && new_r < n && new_c >= 0 && new_c < m &&
                board[new_r][new_c] == word[i] && !is_visited[new_r][new_c]) {
                dfs(is_visited,board,word,i+1,temp,new_r,new_c);
            }
        }
    ...
    bool exist(vector<vector<char>>& board, string word) {
        ...
        rep(i, n, 0) {
            rep(j, m, 0) {
                if (board[i][j] == word[0]) {             
                    dfs(is_visited, board, word, 1, temp, i, j);
                    if (temp)
                        return temp;
                }
            }
