...
{
    return !(n & (n - 1)) && (n & 0x55555555); // check the 1-bit location;
}
ll modinv(ll p, ll q)
{
    ll ex;
...
void solve()
{
    ...
    rep(i, c, 0)
    {
        cin >> ind[i].first >> ind[i].second;
        pre[i + 1] = pre[i] + (ind[i].second - ind[i].first + 1);
    }
    //  rep(i,c+1,0)cout<<pre[i]<<" ";
    //  pe;
    // rep(i,c,0)cout<<ind[i].first<<" "<<ind[i].second<<endl;
    while (q--)
    {
        inll(k);
        int i = 0;
        ...
        int j=lower_bound(all(pre),k)-pre.begin();
        ll req=pre[j]-k;
        // cout<<j<<" ";
        ll curr_ind=ind[j-1].second-req;
        // cout<<curr_ind<<" ";
        while(curr_ind>n){
         int j=lower_bound(all(pre),curr_ind)-pre.begin();
        //  cout<<j<<" ";
         ll req=pre[j]-curr_ind;
         curr_ind=ind[j-1].second-req;
        //  cout<<curr_ind<<" ";
        }
        // pe;
        cout<<x[curr_ind-1]<<endl;
