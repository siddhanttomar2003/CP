...
public:
    int minKBitFlips(vector<int>& nums, int k) {
        int n=nums.size();
        queue<int>q;  // we are using queue to achieve our main goal that is maintain 
        // number of flips for our current window
      int ans=0;
        rep(i,n-k+1,0){
          if(nums[i]==0 && q.size()%2==0){
                 ans++;
                 q.push(i+k-1);// till which index will this flip work
          }
