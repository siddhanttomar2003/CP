...
class Solution {
public:
    void bfs(int i, int j, vvi &grid,vvi &temp){
        map<pi,int>mp;
        queue<pair<pi,int>>q;
        ...
        while(q.size()>0){
            ...
            q.pop();
             rep(k,4,0){
                int nr=sr+delRow[k];
                int nc=sc+delCol[k];
                if(nr>=0 && nr<n && nc>=0 && nc<m && mp.find({nr,nc})==mp.end() && grid[nr][nc]==1 ){
                    mp[{nr,nc}]++;
                    temp[nr][nc]=min(temp[nr][nc],count+1);
    ...
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        rep(i,n,0){
            rep(j,m,0){
                if(grid[i][j]==1 && temp[i][j]==1e8)return -1;    
                if(grid[i][j]==1){
                    // if(temp[i][j]==1e8)return -1;
