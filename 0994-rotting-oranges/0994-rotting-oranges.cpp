...
class Solution {
public:
    void bfs(vvi& grid, int s_i, int s_c, vvi& check) {
        int n = grid.size();
        int m = grid[0].size();
        int delRow[] = {0, -1, 0, 1};
        int delCol[] = {-1, 0, 1, 0};
        ...
        mp[{s_i, s_c}]++;
        while (q.size() > 0) {
            int curr_r = q.front().first.first;
            int curr_c = q.front().first.second;
            ...
            rep(i, 4, 0) {
                ...
                if (n_r >= 0 && n_r < n && n_c >= 0 && n_c < m &&
                    grid[n_r][n_c] == 1 && mp.find({n_r, n_c}) == mp.end()) {
                    check[n_r][n_c] = min(check[n_r][n_c], curr_time+1);
                    mp[{n_r, n_c}]++;
                    q.push({{n_r, n_c},curr_time+1});
                }
            }
        }
    }
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        rep(i, n, 0) {
            rep(j, m, 0) {
                ...
                if(grid[i][j]==1 && check[i][j]==INT_MAX)return -1;
            }
        }
        return ans;
    }
