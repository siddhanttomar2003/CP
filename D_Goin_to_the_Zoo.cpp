...
ll cal(map<ll,vector<ll>>&mp, int i,vector<ll>&v, int n ,vector<int>&vis){
    bool check=true;
    ...
    ans=v[i]+cal(mp,i+1,v,n,vis);
    for(auto it:mp[i]){
        vis[it]--;
    }
    ans=min(ans,cal(mp,i+1,v,n,vis));
    return ans;
}
void solve(){
    inint(n);
