...
class Solution {
public:
    int recur(int n,vector<vvi> &dp,int countA,int countL){
        if(n==0)return 1;
        int take=(recur(n-1,dp,countA,0))%M;// for p
        int take2=0;
        if(dp[n][countA][countL]!=-1)return dp[n][countA][countL];
        if(countA<1){
            take2=(recur(n-1,dp,countA+1,0))%M;
        }
        int take3=0;
        if(countL<2){
          take3=(recur(n-1,dp,countA,countL+1))%M;
        }
        return dp[n][countA][countL]=((take+take2)%M +take3)%M;

    }
    int checkRecord(int n) {
        vector<vvi>dp(n+1,vvi(2,vi(3,-1)));
        return recur(n,dp,0,0);
    }
};