...
int find_max(int i, int budget,vi & prices,vi &pages ,vvi &dp){
    if(i==0){
        if(prices[i]<=budget)return pages[i];
        return 0;
    }
    if(dp[i][budget]!=-1)return dp[i][budget];
    int take=0;int nottake=0;
    if(prices[i]<=budget)take=pages[i]+find_max(i-1,budget-prices[i],prices,pages,dp);
    nottake=find_max(i-1,budget,prices,pages,dp);
...
int32_t main()
{
    ...
    inint(budget);
    vi prices(n);
    vvi dp(n,vi(budget+1,0));
    rep(i,n,0)cin>>prices[i];
    vi pages(n);
    rep(i,n,0)cin>>pages[i];
    rep(i,budget+1,0){
