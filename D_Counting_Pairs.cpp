...
ll check(int m, ll x, ll y, vl &pre, int s, int n)
{
    ll rem_sum = pre[n] - (pre[m] - pre[s - 1]);
    return rem_sum;
}
void solve()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ll mini = tot - y - temp[i];
        ll maxi = tot - x - temp[i];
        auto s = lower_bound(temp.begin() + i + 1, temp.end(), mini);
        auto e = upper_bound(temp.begin() + i + 1, temp.end(), maxi);
        ans += e - s;
    }
    cout << ans << endl;
}
