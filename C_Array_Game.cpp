...
// vector<int>Seg_tree(4*n,0);
   ...
   if(k==1){
     ...
     cout<<ans<<endl;
      ...
      rep(i,n,0){
        rep(j,n,i+1){
            ll curr_diff1=v[j]-v[i];
            auto it=lower_bound(v.begin(),v.end(),curr_diff1);
            if(it!=v.end()){
                ans=min(ans,min(curr_diff1,abs(curr_diff1-*it)));
            }
            auto it2=prev(it);
            if(it!=v.begin()){
                ans=min(ans,min(curr_diff1,abs(curr_diff1-*it2)));
            }
