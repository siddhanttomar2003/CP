...
public:
    vector<vector<int>> buildMatrix(int k, vector<vector<int>>& rowConditions, vector<vector<int>>& colConditions) {
        ...
        vi indegreer(k+1,0);
        rep(i,k+1,1){
            rep(j,adj_r[i].size(),0){
                indegreer[adj_r[i][j]]++;
            }
        }
        rep(i,k+1,1){
            if(indegreer[i]==0)q.push(i);
        }
        while(q.size()>0){
            int num=q.front();
            q.pop();
            topo_r.pb(num);
            rep(i,adj_r[num].size(),0){
                if(indegreer[adj_r[num][i]]>=1){
                    indegreer[adj_r[num][i]]--;
                    if(indegreer[adj_r[num][i]]==0)q.push(adj_r[num][i]);
                }
            }
        }
        // reverse(topo_row.begin(),topo_row.end());
        // rep(i,topo_row.size(),0)cout<<topo_row[i]<<" ";
        // pe;
        if(topo_r.size()!=k)return {};
        vi topo_c;
          queue<int>q1;
        vi indegreec(k+1,0);
        rep(i,k+1,1){
            rep(j,adj_c[i].size(),0){
                indegreec[adj_c[i][j]]++;
            }
        }
        rep(i,k+1,1){
            if(indegreec[i]==0)q1.push(i);
        }
        while(q1.size()>0){
            int num=q1.front();
            q1.pop();
            topo_c.pb(num);
            rep(i,adj_c[num].size(),0){
                if(indegreec[adj_c[num][i]]>=1){
                    indegreec[adj_c[num][i]]--;
                    if(indegreec[adj_c[num][i]]==0)q1.push(adj_c[num][i]);
                }
            }
        }
        if(topo_c.size()!=k)return {};
